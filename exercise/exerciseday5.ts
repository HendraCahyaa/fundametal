...
function gameBatuGuntingKertas(player: "Batu" | "Gunting" | "Kertas") {
  const item = ["Batu", "Gunting", "Kertas"];
  const random = Math.floor(Math.random() * item.length);
  const bot = item[random];

  if (player === bot) {
    return `Player : ${player} VS Bot : ${bot} = Draw`;
  }
  const playerWin =
  ...
  if (playerWin) {
    return `Player : ${player} VS Bot : ${bot} = Player Win`;
  } else {
    return `Player : ${player} VS Bot : ${bot} = Player Lose`;
  }
}
console.log(gameBatuGuntingKertas("Gunting"));
